...
df1 <- df_logit[, c("stablization", "Age", "Dura_Yr", "ISSI_2", "ISSI_2_chg", "Waist_chg", "IS_OGTTU_chg")]
df1 <- na.omit(df1)

m4 <- glm(stablization ~ Age + Dura_Yr + log(ISSI_2) + ISSI_2_chg + Waist_chg + IS_OGTTU_chg, family = binomial(link = "logit"), data = df1)
sum4 <- summary(m4)
...
par(pty = "s")
roc <- roc(df1$stablization, predicted_response, legacy.axes = TRUE, smooth = T, direction = "<")
...
df2 <- df_logit[, c("stablization", "Age", "Dura_Yr", "ISSI_2", "ISSI_2_chg", "BMI_chg", "IS_OGTTU_chg")]
df2 <- na.omit(df2)

m3 <- glm(stablization ~ Age + Dura_Yr + log(ISSI_2) + ISSI_2_chg + BMI_chg + IS_OGTTU_chg, family = binomial(link = "logit"), data = df2)
sum3 <- summary(m3)
...
par(pty = "s")
roc <- roc(df2$stablization, predicted_response, legacy.axes = TRUE, smooth = T, direction = "<")
plot(roc, print.auc = T, legacy.axes = T, main = "ROC Curve")
...
df3 <- df_logit[, c("stablization", "Age", "Dura_Yr", "ISSI_2", "ISSI_2_chg", "BMI_chg", "HOMA_IR_chg")]
df3 <- na.omit(df3)

m1 <- glm(stablization ~ Age + Dura_Yr + log(ISSI_2) + ISSI_2_chg + BMI_chg + HOMA_IR_chg, family = binomial(link = "logit"), data = df3)
sum1 <- summary(m1)
...
par(pty = "s")
roc <- roc(df3$stablization, predicted_response, legacy.axes = TRUE, smooth = T, direction = "<")
plot(roc, print.auc = T, legacy.axes = T, main = "ROC Curve")
...
df7 <- df_logit[, c("stablization", "Age", "Dura_Yr", "ISSI_2", "ISSI_2_chg", "BMI_chg", "HOMA_IR_chg", "avg_phy_act")]
df7 <- na.omit(df7)

m7 <- glm(stablization ~ Age + Dura_Yr + log(ISSI_2) + ISSI_2_chg + BMI_chg + HOMA_IR_chg + avg_phy_act, family = binomial(link = "logit"), 
          data = df7)
sum7 <- summary(m7)
...
kable(summary_tab7, format = "html", caption = "Summary Table for Model III",
      digits = 4) %>%
  kable_paper(full_width = T)
...
par(pty = "s")
roc <- roc(df7$stablization, predicted_response, legacy.axes = TRUE, smooth = T, direction = "<")
plot(roc, print.auc = T, legacy.axes = T, main = "ROC Curve")

cat("AIC of Model I: 54.364")


## 5. Model V: Model III + Change in ALTS form 3-weeks to 2-years
df8 <- df_logit[, c("stablization", "Age", "Dura_Yr", "ISSI_2", "ISSI_2_chg", "BMI_chg", "HOMA_IR_chg", "ALTS_chg")]
df8 <- na.omit(df8)

m8 <- glm(stablization ~ Age + Dura_Yr + log(ISSI_2) + ISSI_2_chg + BMI_chg + HOMA_IR_chg + ALTS_chg, family = binomial(link = "logit"), 
          data = df8)
sum8 <- summary(m8)
...
kable(summary_tab8, format = "html", caption = "Summary Table for Model II",
      digits = 4) %>%
  kable_paper(full_width = T)
...
par(pty = "s")
roc <- roc(df8$stablization, predicted_response, legacy.axes = TRUE, smooth = T, direction = "<")
plot(roc, print.auc = T, legacy.axes = T, main = "ROC Curve")

cat("AIC of Model I: 69.768")
